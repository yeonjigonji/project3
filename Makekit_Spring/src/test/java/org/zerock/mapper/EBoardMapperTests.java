...
@Log4j
public class EBoardMapperTests {
	...
	public void testInsert() {
		...
		board.setTitle("다섯번째 새로 작성하는 글입니다!");
		board.setContent("다섯번째 새로 작성하는 내용입니다!");
		board.setWriter("장나라");
	...
	public void testInsertSelectKey() {
		BoardVO board = new BoardVO();
		board.setTitle("select key~ 네번째 새로 작성하는 글입니다!");
		board.setContent("select key~ 네번째 새로 작성하는 내용입니다!");
		board.setWriter("김희선");
	...
	*/
	...
	public void testSearch() {
		...
		List<EBoardVO> list = mapper.egetListWithPaging(cri);
		list.forEach(board -> log.info(board));
	}
}
