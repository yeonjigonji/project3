...
var replyService = (function() {
	...
	function getList(param, callback, error) {
		...
		$.getJSON("/Ereplies/pages/" + bno + "/" + page + ".json",
				function(data) {
					if(callback){
	...
	// page731 소스 코딩할 때 아래 1줄 소스는 주석 처리해 줍니다.
	// function remove(rno, callback, error) {
	// page731 아래 1줄 소스 코딩 추가
	function remove(rno, replyer, callback, error) {
		$.ajax({
			...
			type : 'delete',
			url : '/Ereplies/delete/' + rno,
			
			// page731 아래 2줄 소스 코딩 추가
	...
	function update(reply, callback, error) {
		...
		$.ajax({
			type : 'put',
			url : '/Ereplies/request/' + reply.rno,
			data : JSON.stringify(reply),
				contentType : "application/json; charset=utf-8",
				success : function(result, status, xhr) {
					if(callback){
	...
	function get(rno, callback, error) {
		$.get("/Ereplies/" + rno + ".json", function(result) {
			if(callback){
				callback(result);
			}
		}).fail(function(xhr, status, err) {
